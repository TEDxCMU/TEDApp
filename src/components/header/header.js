...
export class Header extends Component {
    ...
    render = () => {
        ...
        let headerStyle = styles['header-container'];
        if (this.props.headerStyle === "fixed") {
            if (this.props.link === false) {
                headerStyle = classNames(styles['header-container--sticky'], styles['header-container--sticky-short'])
            } else {
                headerStyle = styles['header-container--sticky']
            }
        } else {
        ...
        return (
            <header className={classNames(headerStyle, styles['header-background'])}>
                ...
                :
                    ...
                    <img src={this.props.image === undefined ? logo : back} className={this.props.image === undefined ? classNames(styles['header__back'], styles['header__back--logo']) : classNames(styles['header__back'], styles['header__back--arrow'])} alt="TEDxCMU"></img>
                    </Link>   
                }
            ...
            <Popup open={this.state.open} closeOnDocumentClick onClose={this.closeModal} contentStyle={popupStyle}>
                <div className={styles['popup']}>
                    <div>
                        ...
                        <textarea type="text" id="iOS" required autoComplete="off" className={this.state.errors === undefined || this.state.errors.question === false ? styles['popup__input'] : classNames(styles['popup__input'], styles['popup__input--invalid'])} name="question" value={that.props.question} placeholder={questionBlank ? "Please write a question before submitting." : "Write your question here..."} onChange={that.props.handleChange} />
                        {this.state.errors === undefined ?
                            null
                            :
                            <small className="text-red">Please ask a question before submitting.</small>}
                        <h4>Sincerely, </h4>
                        <input type="text" className={classNames(styles['popup__input'], styles['popup__input__sm'])} autoComplete="off" value={that.props.name} required minLength="4" size="10" name="name" placeholder={nameBlank ? "Please add your name." : "Your name..."} onChange={that.props.handleChange} />
                        <div className={styles['popup__btn-group']}>
                    ...
                    <div>
                        {this.state.altAnnouncement !== undefined ?
                            <div>
                                ...
                                <textarea type="text" id="iOS" required className={styles['popup__input']} name="announcement" value={this.state.announcement} placeholder={"Make a new custom announcement."} onChange={e => this.handleChange(e)} />
                                <div className={styles['popup__btn-group']}>
                            ...
                            <div>
                                ...
                                <textarea type="text" id="iOS" required className={styles['popup__input']} name="announcement" placeholder={"Make it bold, keep it short!"} onChange={e => this.handleChange(e)} />
                                <div className={styles['popup__btn-group']}>
        ...
        return (
            <div>
                {this.props.link === false ?
                    ...
                    :
                    <div>
                        <h1 className={styles['header__title']}>{this.props.title}</h1>
                        <div>
                            {this.props.description !== undefined && this.state.altAnnouncement === undefined ?
                                <h6 onClick={this.openAnnouncement} className={styles['header__desc']}>{Parser(this.props.description.toString())}</h6>
                                :
                                null
                                }
                            {this.props.description !== undefined && this.state.altAnnouncement !== undefined ?
                                <h6 onClick={this.openAnnouncement} className={styles['header__desc']}>{Parser(this.state.altAnnouncement.toString())}</h6>
                                :
                                null
                                }
                        </div>
            ...
            <div className={styles['header__profile']}>
                {this.props.image !== undefined ?
                    <div className="relative">
                        <div className={styles['header__img--border']}></div>
                        <img className={styles['header__img']} alt="speaker" src={this.props.image}></img>
                    </div>
                    :
                    null}
                <br />
                <h1 className={classNames(styles['header__title'], styles['header__title--speaker'])}>{this.props.title}</h1>
                {this.props.tag !== undefined ?
                    <div>
                        <h6 className={classNames(styles['header__desc'], styles['header__desc--speaker'], 'text-center')}>{this.props.tag}</h6>
                    </div>
                    :
                    null}
    ...
    collapsedHomeHeader = () => {
        return (
            <div>
                {this.props.description !== undefined && this.state.altAnnouncement === undefined ?
                    <div>
                        <h6 onClick={this.openAnnouncement} className={classNames(styles['header__desc'], styles['header__desc--collapsed'])}>{Parser(this.props.description.toString())}</h6>
                    </div>
                    :
                    null
                    }
                ...
                {this.props.description !== undefined && this.state.altAnnouncement !== undefined ?
                    <div>
                        <h6 onClick={this.openAnnouncement} className={classNames(styles['header__desc'], styles['header__desc--collapsed'])}>{Parser(this.state.altAnnouncement.toString())}</h6>
                    </div>
                    :
                    null
                    }
